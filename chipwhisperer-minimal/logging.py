...
# Default Chipwhisperer Logger, leave this for now.
log_dir = tempfile.mkdtemp(prefix="chipwhisperer")

logging.basicConfig(level=logging.WARNING)
cw_formatter = logging.Formatter(
    "(%(name)s %(levelname)s|File %(filename)s:%(lineno)d) %(message)s"
)
...
for logger in chipwhisperer_loggers:
    ...
    filehndlr = logging.FileHandler(
        os.path.join(log_dir, "chipwhisperer/logs", logger.name + ".log"), mode="w"
    )
...
naeusb_logger.handlers[1].setLevel(
    logging.INFO
)  # only log info for this one, as files get big quickly
